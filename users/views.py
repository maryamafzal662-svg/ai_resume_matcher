...
class CandidateSearchView(APIView):
    ...
    def get(self, request):
        ...
        serializer = CustomUserSerializer(candidates[:10], many=True) 
        return Response(serializer.data)
...
class ChatbotMessageAPIView(APIView):
    ...
    def post(self, request):
        ...
        # ---------------- COMMON RULES (Greetings etc.) ----------------
        rules_common = {
            ("hello", "hi", "hey", "hy", "helo", "heyy", "hyy", "good morning", "good evening", "good afternoon"):
        ...
        rules_jobseeker = {
            ...
            ("website", "site", "platform", "about", "introduction", "intro", "how website works",
             ...
             "explain portal", "explain system", "tell me site intro"):
                "Welcome to the AI Job Matching Platform! As a jobseeker, you can create and update your resume, "
                "browse job listings, receive smart recommendations, apply with a cover letter, and track your application status.",
            ...
            ("profile", "my profile", "show profile", "view profile", "check profile", "profile info",
             "account details", "account info", "see my details", "profile section"):
                "Your profile contains your username, email, phone, location, role, and join date. You can also upload a profile picture.",

            ("edit profile", "update profile", "change profile", "modify profile", "update account", "change info",
             "edit account", "modify account details"):
                "To update your profile, go to Dashboard → Profile → Click 'Edit Profile' → Make your changes → Save Changes.",

            # --- Resume ---
            ("resume", "cv", "my resume", "show resume", "view resume", "display resume", "my cv", "cv info"):
                "You can manage your resume in the 'My Resume' section. Create, edit, or delete your resume as needed.",

            ("create resume", "add resume", "make resume", "build resume", "create cv", "add cv", "make cv", "build cv"):
                "To create a resume: Go to My Resume → Click 'Create Resume' → Enter profession, education, experience, and skills → Click 'Save'.",

            ("update resume", "edit resume", "change resume", "modify resume", "update cv", "edit cv"):
                "To update your resume: Go to My Resume → Click 'Edit' → Modify details → Click 'Update Resume'.",

            ("delete resume", "remove resume", "erase resume", "delete cv", "remove cv"):
                "If you want to delete your resume, go to My Resume → Click 'Delete'.",

            # --- Jobs ---
            ("jobs", "browse jobs", "job listings", "find jobs", "job search", "explore jobs", "available jobs"):
                "All available jobs are shown in the 'Job Listings' section. You can access it from the navbar or dashboard.",

            ("apply", "apply job", "apply now", "job apply", "submit application", "how to apply", "send application"):
                "To apply for a job: Go to Job Listings → Click 'View Details' → Click 'Apply Now' → Write your cover letter → Submit Application. "
                "Once submitted, the button will turn grey and you cannot apply again.",

            # --- Recommendations ---
            ("recommendations", "recommended jobs", "suggested jobs", "job recommendations", "matching jobs", "smart jobs"):
                "Based on your resume and skills, our system suggests matching jobs with a job score percentage. "
                "You can view them under the 'Recommendations' section.",

            # --- Applications ---
            ("applications", "my applications", "submitted jobs", "applied jobs", "job applications"):
                "In the Applications section, you can see all jobs you have applied for along with their status (Accepted/Rejected/Pending).",

            # --- Offers ---
            ("job offers", "offers", "my offers", "received offers"):
                "Employers can send you job offers. You can view them under 'Job Offers' and either Accept or Reject. "
                "If you accept, the company will contact you by email.",

            # --- Notifications ---
            ("notifications", "alerts", "updates", "messages", "my notifications"):
                "All your alerts and updates are available in the Notifications section.",

            # --- Navigation ---
            ("dashboard", "home", "main page", "control panel", "user dashboard"):
                "Your dashboard shows your profile, resume, recommendations, job applications, and offers at a glance.",

            ("logout", "sign out", "log out", "exit", "end session"):
                "To securely log out, click 'Logout' from the navbar.",
        }
        ...
        rules_employer = {
            # --- Website Intro ---
            ("website", "site", "platform", "about", "introduction", "intro", "how website works",
             "what is this website", "tell me about website", "guide", "help", "employer intro", "employer guide"):
                "Welcome to the AI Job Matching Portal for Employers! You can create your company profile, "
                "post jobs, review candidates, and manage applications directly from your dashboard.",

            # --- Profile ---
            ("profile", "my profile", "show profile", "view profile", "check profile", "profile info",
             "account details", "account info", "see my details", "profile section"):
                "Your profile contains your username, email, phone, role, location, and join date. You may also upload a profile image.",

            ("edit profile", "update profile", "change profile", "modify profile", "update account"):
                "To edit your profile: Dashboard → Profile → Edit Profile → Save Changes.",

            # --- Company ---
            ("company", "my company", "create company", "add company", "company details"):
                "In 'My Company', you can create your company profile by adding company name, industry, website, location, and description. "
                "Once created, you can edit or delete it later.",

            ("delete company", "remove company", "erase company", "delete organization"):
                "Go to My Company → Click 'Delete' to remove your company profile. You can create it again later.",

            # --- Job Posting ---
            ("post job", "add job", "create job", "job posting", "post new job", "make job"):
                "To post a job: Go to Post Job → Enter title, description, location, salary, expiry date, category, and skills → Submit Job.",

            ("job management", "manage jobs", "job list", "view jobs", "posted jobs"):
                "In Job Management, you can see all your posted jobs and manage them (edit or delete).",

            ("edit job", "update job", "modify job", "change job posting"):
                "Go to Job Management → Select the job → Edit details → Save Changes.",

            ("delete job", "remove job", "erase job", "cancel job posting"):
            ...
            ("applications", "view applications", "job applications", "candidates", "applicants"):
                "In the Applications section, you can view all jobseekers who applied to your jobs and check their resumes.",

            ("accept application", "approve application", "hire", "accept candidate"):
                "To accept an application: Applications → Review Resume → Click 'Accept'.",

            ("reject application", "decline application", "reject candidate", "not hire"):
                "To reject an application: Applications → Review Resume → Click 'Reject'.",

            ("candidate resume", "view resume", "check resume", "applicant cv", "review resume"):
                "You can view a candidate’s resume by selecting them from the Applications section.",

            # --- Candidates & Job Offers ---
            ("candidates", "job seekers", "all candidates", "available jobseekers"):
                "The Candidates section shows all jobseekers. You can view their profiles and send job offers.",

            ("job offer", "send offer", "offer job", "give job offer"):
                "To offer a job: Go to Candidates → Select Jobseeker → Click 'Offer Job' → Fill job details → Submit. "
                "The jobseeker can accept or reject, and you’ll see their response in status.",

            # --- Notifications ---
            ("notifications", "alerts", "updates", "messages", "my notifications"):
                "All employer notifications, including new applications and responses, are shown in the Notifications section.",

            # --- Navigation ---
            ("dashboard", "home", "main page", "control panel", "employer dashboard"):
                "Your dashboard displays profile info, company details, job postings, and applications summary.",
        ...
        # ---------------- RULE SELECTION ----------------
        rules = {}
        rules.update(rules_common)  # add greetings
        rules.update(rules_employer if role == "employer" else rules_jobseeker)
